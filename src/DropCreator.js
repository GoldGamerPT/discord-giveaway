...
class DropCreator extends EventEmitter {
    ...
    constructor(client, url = '') {
        ...
        this.client.on('message', async message => {
            ...
            if (randomNumber === 17) {
                ...
                .setTitle(`${prize}`)
                .setDescription(`O primeiro a reagir com 🎉 ganha o prémio.`)
                ...
                reaction.on('collect', async (reaction, user) => {
                    ...
                    embed.setTitle(`🎉 Vencedor!`);
                    embed.setDescription(`${user.toString()} ganhou \`${prize}\`. Abre um ticket para receberes o teu prémio!`);
                    
                    await msg.edit({ embeds: [embed] });

                    msg.channel.send(`${user.toString()} ganhou **${prize}**`);
    ...
    async createDrop(options) { 
        if (!options.prize) throw new Error("Põe o prémio!");
